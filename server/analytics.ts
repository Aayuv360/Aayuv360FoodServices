...
export class AnalyticsService {
  ...
  private getDateRange(range: AnalyticsDateRange): { startDate: Date, endDate: Date } {
  ...
  async getAnalytics(dateRange: AnalyticsDateRange): Promise<AnalyticsResponse> {
    ...
    const prevOrders = await this.getOrdersInDateRange(prevStartDate, prevEndDate);
    ...
    const categoryDistribution = await this.generateCategoryDistribution(allOrders);
    ...
    const orderTimeDistribution = await this.getOrderTimeDistribution(allOrders);
  ...
  private async getOrdersInDateRange(startDate: Date, endDate: Date): Promise<Order[]> {
    try {
      // In a real implementation, you would filter orders by date from the database
      // For now, we'll work with the in-memory storage
  ...
  private async getUsersInDateRange(startDate: Date, endDate: Date): Promise<User[]> {
    try {
      // In a real implementation, you would filter users by creation date
  ...
  private countNewCustomers(users: User[], startDate: Date, endDate: Date): number {
  ...
  private async generateRevenueData(dateRange: AnalyticsDateRange, orders: Order[]): Promise<RevenueData[]> {
    ...
    if (dateFormat === 'day') {
      ...
      orders.forEach(order => {
        ...
        weekStart.setDate(orderDate.getDate() - orderDate.getDay()); // Set to beginning of week (Sunday)
      ...
      // Sort weeks and calculate revenue
      ...
      // Sort months and calculate revenue
  ...
  private async generateCategoryDistribution(orders: Order[]): Promise<CategoryDistribution[]> {
    // This would typically join orders with order items and meals to get categories
    // For our mock data, we'll create sample distribution
    const categoryMap = new Map<string, { orders: number, revenue: number }>();
    
    // For a real implementation, you'd need to retrieve all order items and join with meals
    ...
    // Create mock distribution based on order counts
    ...
    if (totalOrders > 0) {
      ...
      categoryMap.set('Bajra (Pearl Millet)', { 
        ...
        revenue: Math.floor(this.calculateTotalRevenue(orders) * 0.20) 
      });
  ...
  private async getTopMeals(orders: Order[]): Promise<TopMeal[]> {
    // In a real implementation, you would join orders with order items and meals
    // to get the most ordered meals
  ...
  private async generateUserActivity(dateRange: AnalyticsDateRange, users: User[]): Promise<UserActivity[]> {
    const result: UserActivity[] = [];
    const { startDate, endDate } = this.getDateRange(dateRange);
    
    // Define the interval for data points
    ...
    if (interval === 'daily') {
      ...
      while (current <= endDate) {
        ...
        // In a real implementation, you would check user activity records
        // For now, we'll use a formula to simulate active users
      ...
      // Weekly data points logic would go here
      // Similar to the revenue weekly calculation
    } else {
      // Monthly data points logic would go here
      // Similar to the revenue monthly calculation
    }
    
    return result;
  }
  
  private async calculateRetention(users: User[], orders: Order[]): Promise<RetentionData[]> {
    // In a real implementation, this would involve complex queries to track cohorts
    // For now, we'll return sample cohort data
    return [
      {
        cohort: 'January 2023',
  ...
  private async getSubscriptionStats(subscriptions: Subscription[]): Promise<SubscriptionStats[]> {
    // Group subscriptions by type and calculate counts and revenue
  ...
  private async getOrderTimeDistribution(orders: Order[]): Promise<OrderTimeDistribution[]> {
    // Define time slots
    const timeSlots = [
      '12-2 PM', '2-4 PM', '4-6 PM', '6-8 PM', '8-10 PM', '10-12 PM'
    ];
    
    // Create a map to store counts for each time slot
  ...
  private async getLocationData(users: User[], orders: Order[]): Promise<LocationData[]> {
    // In a real implementation, you would use address data from users and orders
    // For now, we'll return sample location data for Hyderabad areas
    return [
