...
router.get('/api/notifications/user/:userId', isAuthenticated, async (req, res) => {
  try {
...
router.put('/api/notifications/:notificationId/read', isAuthenticated, async (req, res) => {
  try {
...
router.put('/api/notifications/user/:userId/read-all', isAuthenticated, async (req, res) => {
  try {
    const userId = parseInt(req.params.userId);
    if (isNaN(userId)) {
      return res.status(400).json({ error: 'Invalid user ID' });
...
router.post('/api/notifications/send/app', isAuthenticated, async (req, res) => {
  try {
...
router.post('/api/notifications/send/sms', isAuthenticated, async (req, res) => {
  try {
    const { userId, title, message } = req.body;
...
router.post('/api/notifications/send/whatsapp', isAuthenticated, async (req, res) => {
  try {
    const { userId, title, message } = req.body;
    ...
    const notification = await sendWhatsAppNotification(userId, title, message);
    res.json(notification);
  } catch (error) {
