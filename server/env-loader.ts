...
export function loadEnvironment() {
  ...
  if (result.error) {
    ...
    if (NODE_ENV === "production") {
      console.error(
        "Production environment requires .env.production file or environment variables set in deployment platform",
      );
      process.exit(1);
    }
  } else {
...
function validateEnvironment() {
  ...
  if (NODE_ENV === "production") {
    ...
    if (prodMissing.length > 0) {
      console.error(
        `❌ Missing production environment variables: ${prodMissing.join(", ")}`,
      );
      process.exit(1);
    }
  }
